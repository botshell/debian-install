...
如果你在配置“服务器”（Server）： 强烈建议保持默认的 C.UTF-8（或者 en_US.UTF-8）。因为服务器报错时，英文日志在网上（如 Stack Overflow）更容易搜到解决方案，而且能避免很多老旧脚本因为不认中文路径或中文提示而崩溃。

如果你在配置“日常桌面电脑”（Desktop）：
建议选择 zh_CN.UTF-8。因为你肯定希望整个系统的菜单、桌面图标、软件界面都是亲切的中文，看起来不费劲。
